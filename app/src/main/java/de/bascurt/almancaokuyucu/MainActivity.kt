...
private fun SentenceBuildStudyScreen(items: List<Lexeme>, appLanguage: String, translationLanguage: String, onBack: () -> Unit, onAnswered: (Lexeme, Boolean) -> Unit) {
    ...
    StudyHeader(uiText(appLanguage, "Cümle Kur"), index, cases.size, correctCount, onBack) {
            ...
            if (chosen.isEmpty()) {
                ...
                FlowRow(
                    ...
                    chosen.forEachIndexed { chosenIndex, word ->
                        var dragX by remember(index, chosenIndex) { mutableStateOf(0f) }
                        val density = LocalDensity.current
                        Surface(
                            shape = RoundedCornerShape(11.dp),
                            color = Turquoise.copy(alpha = .16f),
                            contentColor = MaterialTheme.colorScheme.onSurface,
                            tonalElevation = if (dragX != 0f) 6.dp else 0.dp,
                            modifier = Modifier
                                .height(42.dp)
                                .offset { IntOffset(dragX.roundToInt(), 0) }
                                .pointerInput(index, chosenIndex) {
                                    detectDragGesturesAfterLongPress(
                                        onDragStart = { dragX = 0f },
                                        onDragCancel = { dragX = 0f },
                                        onDragEnd = {
                                            val stepPx = with(density) { 72.dp.toPx() }
                                            val shift = (dragX / stepPx).roundToInt()
                                            val targetIndex = (chosenIndex + shift).coerceIn(0, chosen.lastIndex)
                                            if (targetIndex != chosenIndex) {
                                                val updated = chosen.toMutableList()
                                                val moving = updated.removeAt(chosenIndex)
                                                updated.add(targetIndex, moving)
                                                chosen = updated
                                            }
                                            dragX = 0f
                                        },
                            ...
                            Box(
                                ...
                                Text(word, fontSize = 14.sp, fontWeight = FontWeight.Medium, maxLines = 1)
                            }
                        }
                    }
                }
        ...
        Text("Kelimeyi uzun basıp sürükle; bırakınca yeni yerine oturur.", color = MaterialTheme.colorScheme.onSurfaceVariant, fontSize = 11.sp)
        Spacer(Modifier.height(6.dp))
        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(7.dp)) {
            OutlinedButton(
                onClick = {
        ...
        FlowRow(
            ...
            sourceWords.forEachIndexed { wordIndex, word ->
                ...
                AssistChip(
                    ...
                    label = { Text(word, fontSize = 14.sp, fontWeight = FontWeight.Medium, maxLines = 1) },
...
@Composable private fun StudyHeader(title: String, index: Int, total: Int, correct: Int, onBack: () -> Unit, content: @Composable ColumnScope.() -> Unit) {
    Column(Modifier.fillMaxSize().background(MaterialTheme.colorScheme.background)) {
        Column(Modifier.fillMaxWidth().background(Dark).statusBarsPadding().padding(horizontal = 14.dp, vertical = 10.dp)) {
            Row(Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) {
                Text("‹", color = Color.White, fontSize = 28.sp, modifier = Modifier.clickable(onClick = onBack).padding(end = 10.dp))
                Text(title, color = Color.White, fontSize = 20.sp, fontWeight = FontWeight.Bold, modifier = Modifier.weight(1f))
                Text("${index + 1}/$total", color = Color(0xFFD5E4E8), fontSize = 13.sp)
            }
            Spacer(Modifier.height(6.dp))
            LinearProgressIndicator(progress = { (index + 1).toFloat() / total.coerceAtLeast(1) }, modifier = Modifier.fillMaxWidth())
            Spacer(Modifier.height(4.dp))
            Text("Doğru: $correct", color = Color(0xFFD5E4E8), fontSize = 12.sp)
        }
        Column(
            Modifier
                .fillMaxSize()
